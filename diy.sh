...
make olddefconfig > /dev/null 2>&1 || warn "make olddefconfig 有警告（通常可忽略）"
ok "[0/7] 内核配置注入完成"
...
git clone --depth=1 https://github.com/kzio111/OpenWrt-for-XG-040G-MD.git package/temp_npu > /dev/null 2>&1 || fail "NPU 仓库克隆失败"
cp -r package/temp_npu/package/luci-app-airoha-npu package/ || fail "复制 NPU 插件失败"
rm -rf package/temp_npu

MAKEFILE="package/luci-app-airoha-npu/Makefile"
if [ -f "$MAKEFILE" ]; then
    sed -i 's/LUCI_DEPENDS:=.*/LUCI_DEPENDS:=+luci-base +busybox @TARGET_airoha/' "$MAKEFILE" && ok "LUCI_DEPENDS 修复成功"
    if ! grep -q "chmod 0755" "$MAKEFILE"; then
        sed -i '/define Package\/luci-app-airoha-npu\/install/,/endef/ s/$(call LuCI\/Install.*/&\n\tchmod 0755 $(1)\/usr\/libexec\/rpcd\/luci.airoha_npu/' "$MAKEFILE" && ok "chmod 0755 修复成功"
    fi
fi
...
# 7. 配置锁定（直接修改 .config）
# =========================================================
echo -e "${BLUE}[7/7] 锁定 .config 配置...${NC}"
